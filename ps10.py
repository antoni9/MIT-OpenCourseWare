...
class Hand(object):
    ...
    def update(self, word):
        ...
        for letter in wordDict:
            if letter in self.handDict:
                if wordDict[letter] == self.handDict[letter]:
                    del self.handDict[letter]
                elif wordDict[letter] < self.handDict[letter]:
                    self.handDict[letter] = self.handDict[letter] - wordDict[letter]
                else:
                    return ValueError('Value Error in Hand.update')
    ...
    def containsLetters(self, letters):
        ...
        """
        # TODO
        lettersDict = getFrequencyDict(letters)
        for letter in lettersDict:
            if letter not in self.handDict or lettersDict[letter] > self.handDict[letter]:
                return False
        return True
    ...
    def __eq__(self, other):
        ...
        """
        # TODO
        if set(self.handDict.values()) != set(other.handDict.values()) or \
                set(self.handDict.keys()) != set(other.handDict.keys()):
            return False
        else:
            return True
...
class ComputerPlayer(Player):
    ...
    def pickBestWord(self, wordlist):
        ...
        """
        # TODO
        handSet = ''.join(sorted(str(self.hand))).lstrip()
        bestGuess = 'aa'
        ...
        for m in range(2, 8):
            ...
            for i in handComb:
                guess = ''.join(sorted(i))
                if guess in wordlist.wordPointsDict:
                    if wordlist.getDict()[guess][0] > wordlist.getDict()[''.join(sorted(bestGuess))][0]:
                        bestGuess = wordlist.wordPointsDict[guess][1]
        return bestGuess
...
class Wordlist(object):
    ...
    def __init__(self):
        ...
        for word in self.wordlist:
            self.wordPointsDict[''.join(sorted(word))] = (getWordScore(word), word)
    ...
    def getDict(self):
        return self.wordPointsDict

# wordlist = Wordlist()
# hand = Hand(8)
# pc = ComputerPlayer(1, hand)
# guess = pc.pickBestWord(wordlist)
# print(guess)
