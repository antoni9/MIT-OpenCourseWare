...
class ModeFrame(wx.Frame):
    ...
    def OnSolo(self, event):
        ...
        """
        self.Close()
        PlayFrame(ps10.HUMAN_SOLO).Show()
    def OnVsComp(self, event):
        """
        Start a vs. computer game.
        """
        self.Close()
        PlayFrame(ps10.HUMAN_VS_COMP).Show()
    def OnVsHuman(self, event):
        """
        Start a vs. human game.
        """
        self.Close()
        PlayFrame(ps10.HUMAN_VS_HUMAN).Show()
...
class PlayFrame(wx.Frame):
    ...
    def StartGame(self):
        ...
        hand.
        """
        wordlist = ps10.Wordlist()
        self.game = ps10.Game(self.mode, wordlist)
        self.StartHand()
    ...
    def StartHand(self):
        ...
        wx.MessageDialog(self,
                ...
                caption = 'Get Ready!',
                style = wx.OK).ShowModal()
        if type(player) == ps10.ComputerPlayer:
            # Disable controls.
