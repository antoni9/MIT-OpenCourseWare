...
def get_word_score(word, n):
    ...
    if n == 0:
        ...
        return x
    for i in word:
        x = x + SCRABBLE_LETTER_VALUES.get(i)
...
def play_hand(hand, word_list):
    ...
    * The final score is displayed.
    ...
    """
    I forgot the previously completed Problem#2 and the provided display_hand() function
    So I wrote everything in a while loop using a list to manage the game length and to display hand
    There's too many dependencies this way, it's not ideal
    """
    ...
    while len(newHand) > 1:
        print('make a word using these letters: ', ' '.join(newHand))
        guess = input()
        if guess == '.':
        ...
        if is_valid_word(guess, hand, word_list) == False:
            ...
            wordScore = get_word_score(guess, len(guess))
            print('you scored', wordScore, 'points for this word')
            score = score + wordScore
            print('your total score is: ', score)
            guessDict = {}
            for x in guess:
                guessDict[x] = guessDict.get(x,0) + 1
            for k in guessDict:
                hand[k] = hand[k] - guessDict[k]
            for m in guess:
                newHand.remove(m)
    return score
